          ...
          if (!global._firebaseAdminInitialized) {
            ...
            if (process.env.FIREBASE_PRIVATE_KEY && process.env.FIREBASE_CLIENT_EMAIL) {
              ...
              const serviceAccount = {
                ...
                client_id: process.env.FIREBASE_CLIENT_ID || process.env.GOOGLE_CLIENT_ID,
              ...
              console.log('🔄 Intentando inicializar Firebase con GOOGLE_* ...');
              initializeApp({
                credential: cert({
                  type: "service_account",
...
const KEEP_LATEST_SPECS = [
  {
    collection: 'pedidos',
    dateFields: ['fechaPedido', 'createdAt', 'fechaCreacion'],
    logs: { collection: 'pedidos_logs', parentField: 'pedidoId' }
  },
  {
    collection: 'cobros',
    dateFields: ['fechaCobro', 'createdAt', 'fechaCreacion'],
    logs: { collection: 'cobros_logs', parentField: 'cobroId' }
  },
  {
    collection: 'hojasDeRuta',
    dateFields: ['fecha', 'fechaCreacion', 'createdAt']
  }
];
...
async function planKeepLatest(days) {
  ...
  for (const spec of KEEP_LATEST_SPECS) {
    ...
    let logsToDelete = 0;
    if (spec.logs) {
      const logsSnapshot = await adminDb.collection(spec.logs.collection).get();
      logsToDelete = logsSnapshot.docs.filter((logDoc) => {
        const parentId = logDoc.data()?.[spec.logs.parentField];
        return parentId && !keepIds.has(parentId);
      }).length;
    }
...
async function executeKeepLatest(days) {
  ...
  for (const spec of KEEP_LATEST_SPECS) {
    ...
    if (spec.logs) {
      const logsSnapshot = await adminDb.collection(spec.logs.collection).get();
      const logRefs = logsSnapshot.docs
        .filter((logDoc) => {
          const parentId = logDoc.data()?.[spec.logs.parentField];
          return parentId && !keepIds.has(parentId);
        })
        .map((logDoc) => logDoc.ref);
      logsDeleted = await deleteRefsInBatches(logRefs);
    }
...
async function autoKeepLatestOperativos() {
  if (!adminDb) {
    console.warn('[AUTO-CLEANUP] Firebase no inicializado, omitiendo recorte');
    return;
  }
  try {
    ...
    console.error('[AUTO-CLEANUP] Error recortando colecciones operativas:', error);
  }
}
...
app.listen(PORT, () => {
  ...
  const autoCleanupDisabled = ['0', 'false', 'no', 'off'].includes(
    String(process.env.AUTO_CLEANUP_KEEP_LATEST || 'true').trim().toLowerCase()
  );
